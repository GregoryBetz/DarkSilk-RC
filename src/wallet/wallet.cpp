...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int32_t& nChangePos, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay, const CWalletTx* wtxInOffer, const CWalletTx* wtxInCert, const CWalletTx* wtxInAccount, const CWalletTx* wtxInEscrow, bool darksilkTx)
{
    ...
    CAmount nValue = 0;
    // SERVICES: get output amount of input transactions for service/marketplace calls
    int nTxOutOffer = 0;
    int nTxOutCert = 0;
    int nTxOutAccount = 0;
    int nTxOutEscrow = 0;
    ...
    wtxNew.BindWallet(this);
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                // SERVICES input credit from input tx
                CAmount nWtxinCredit = 0;
                if(wtxInOffer != NULL)
                ...
                // SERVICES add input credit to current coin selection
                CAmount nValueToSelect = nValue - nFeeRet;
                CAmount nValueWithCredit = nValueToSelect - nWtxinCredit;
                if (nValueWithCredit > 0 && !SelectCoinsForServices(nValueWithCredit, wtxNew.nTime, setCoins, nValueIn, coinControl))
                {
