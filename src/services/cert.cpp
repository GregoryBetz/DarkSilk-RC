...
#include "services/messagecrypter.h"
...
bool DecryptMessage(const vector<unsigned char> &vchPubKey, const vector<unsigned char> &vchCipherText, string &strMessage)
{
	...
	if (!pwalletMain->GetKey(pubKeyID, PrivateKey))
        return false;
	CDarkSilkSecret Secret(PrivateKey);
...
UniValue certnew(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 3 || params.size() > 4)
        throw runtime_error(
		"certnew <account> <title> <data> [private=0]\n"
	...
	if(bPrivate)
	{
		...
		if(!EncryptMessage(account.vchPubKey, vchData, strCipherText))
		{
	...
	newCert.vchPubKey = account.vchPubKey;
	...
	SendMoneyForServices(vecSend, recipient.nAmount+fee.nAmount, false, wtx, wtxInOffer, wtxInCert, wtxInAccount, wtxInEscrow);
...
UniValue certupdate(const UniValue& params, bool fHelp) {
	...
	if (wtxIn == NULL || !IsDarkSilkTxMine(tx, "cert"))
	...
	const CWalletTx * wtxInAccount=NULL;
	const CWalletTx * wtxInEscrow=NULL;
	SendMoneyForServices(vecSend, recipient.nAmount+fee.nAmount, false, wtx, wtxInOffer, wtxIn, wtxInAccount, wtxInEscrow);	
 	UniValue res(UniValue::VARR);
	res.push_back(wtx.GetHash().GetHex());
	return res;
}
...
UniValue certtransfer(const UniValue& params, bool fHelp) {
 if (fHelp || params.size() != 2)
        throw runtime_error(
		"certtransfer <certkey> <account>\n"
                "<certkey> certificate guidkey.\n"
				"<account> Account to transfer this certificate to.\n"
                 + HelpRequiringPassphrase());
	...
	catch(...)
	{
		...
		if (!myAddress.IsValid())
			throw runtime_error("Invalid darksilk address");
		if (!myAddress.isAccount)
			throw runtime_error("You must transfer to a valid account");

		// check for account existence in DB
		vector<CAccountIndex> vtxAccountPos;
		if (!paccountdb->ReadAccount(vchFromString(myAddress.accountName), vtxAccountPos))
			throw runtime_error("failed to read account from account DB");
		if (vtxAccountPos.size() < 1)
			throw runtime_error("no result returned");
		CAccountIndex xferAccount = vtxAccountPos.back();
		vchPubKeyByte = xferAccount.vchPubKey;
		xferKey = CPubKey(vchPubKeyByte);
		if(!xferKey.IsValid())
		{
	...
	if (wtxIn == NULL || !IsDarkSilkTxMine(*wtxIn, "cert"))
...
UniValue certinfo(const UniValue& params, bool fHelp) {
	...
	if (!GetDarkSilkTransaction(ca.nHeight, ca.txHash, tx, Params().GetConsensus()))
    ...
    oCert.push_back(Pair("ismine", IsDarkSilkTxMine(tx, "cert") ? "true" : "false"));
	...
	oCert.push_back(Pair("account", address.accountName));
...
UniValue certlist(const UniValue& params, bool fHelp) {
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet)
    {
		...
		else
		{
			cert = vtxPos.back();
			CTransaction tx;
			if (!GetDarkSilkTransaction(cert.nHeight, cert.txHash, tx, Params().GetConsensus()))
				continue;
			if (!DecodeCertTx(tx, op, nOut, vvch) || !IsCertOp(op))
				continue;
			if(!IsDarkSilkTxMine(tx, "cert"))
				continue;
		}
		...
		oName.push_back(Pair("account", address.accountName));
		expired_block = nHeight + GetCertExpirationDepth();
...
UniValue certfilter(const UniValue& params, bool fHelp) {
	...
	BOOST_FOREACH(pairScan, certScan) {
        ...
        if (nCountFrom < nFrom + 1)
            continue;
        CTransaction tx;
		if (!GetDarkSilkTransaction(txCert.nHeight, txCert.txHash, tx, Params().GetConsensus()))
			continue;
...
UniValue certscan(const UniValue& params, bool fHelp) {
    ...
    BOOST_FOREACH(pairScan, certScan) {
        ...
        vector<unsigned char> vchValue = txCert.vchTitle;
        string value = stringFromVch(vchValue);
		if (!GetDarkSilkTransaction(nHeight, txCert.txHash, tx, Params().GetConsensus()))
			continue;
