...
public:
	std::vector<unsigned char> accountLinkVchRand;
	char nDiscountPct;
    ...
    void SetNull() { accountLinkVchRand.clear(); nDiscountPct = 0;}
    bool IsNull() const { return (accountLinkVchRand.empty() && nDiscountPct == 0); }
...
public:
    ...
    bool GetLinkEntryByHash(const std::vector<unsigned char> &ahash, COfferLinkWhitelistEntry &entry) {
    	for(unsigned int i=0;i<entries.size();i++) {
    		if(entries[i].accountLinkVchRand == ahash) {
    			entry = entries[i];
    			return true;
    		}
    	}
    ...
    void PutWhitelistEntry(const COfferLinkWhitelistEntry &theEntry) {
    	for(unsigned int i=0;i<entries.size();i++) {
    		...
    		if(theEntry.accountLinkVchRand == entry.accountLinkVchRand) {
    			entries[i] = theEntry;
    			return;
    		}
    	}
