...
bool CheckEscrowInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock* block) {
	...
	vector<vector<unsigned char> > vvchPrevArgs, vvchPrevAccountArgs;
	if(fJustCheck)
	{
		// Strict check - bug disallowed
    ...
    if (tx.nVersion != DARKSILK_TX_VERSION) {
		LogPrintf("CheckEscrowInputs() : non-darksilk transaction\n");
        return true;
    }
	...
	if(theEscrow.vchWhitelistAccount.size() > MAX_ID_LENGTH)
	{
		return error("escrow account guid too long");
	}
	...
	if(fJustCheck)
	{
		switch (op) {
			case OP_ESCROW_ACTIVATE:
				if(!theEscrow.vchWhitelistAccount.empty() && !IsAccountOp(prevAccountOp))
					return error("CheckEscrowInputs() : an escrow offer accept using an account must attach the account as an input");
				if(IsAccountOp(prevAccountOp) && vvchPrevAccountArgs[0] != theEscrow.vchWhitelistAccount)
					return error("CheckEscrowInputs() : escrow account guid and account input guid mismatch");
				break;
			case OP_ESCROW_RELEASE:
...
UniValue escrownew(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 5 )
        throw runtime_error(
		"escrownew <account> <offer> <quantity> <message> <arbiter account>\n"
						...
						"<arbiter account> Account of Arbiter.\n"
	...
	CDarkSilkAddress arbiterAddress = CDarkSilkAddress(strArbiter);
	if (!arbiterAddress.IsValid())
		throw runtime_error("Invalid arbiter darksilk address");
	if (!arbiterAddress.isAccount)
		throw runtime_error("Arbiter must be a valid account");
	if(IsMine(*pwalletMain, arbiterAddress.Get()))
		throw runtime_error("Arbiter account must not be yours");
	// check for account existence in DB
	vector<CAccountIndex> vtxPos;
	if (!paccountdb->ReadAccount(vchFromString(arbiterAddress.accountName), vtxPos))
		throw runtime_error("failed to read account from account DB");
	if (vtxPos.size() < 1)
		throw runtime_error("no result returned");
	CAccountIndex arbiteraccount = vtxPos.back();
	...
	CAccountIndex buyeraccount;
	CTransaction accounttx;
	if (!GetTxOfAccount(vchAccount, buyeraccount, accounttx))
	...
	CScript scriptPubKeyAccount, scriptPubKeyAccountOrig;
	...
	for(unsigned int i=0;i<theOffer.linkWhitelist.entries.size();i++) {
		...
		{
			// check for existing account updates/transfers
			if (ExistsInMempool(entry.accountLinkVchRand, OP_ACCOUNT_UPDATE)) {
				throw runtime_error("there is are pending operations on that account");
			}
			// make sure its in your wallet (you control this account)
			wtxAccountIn = pwalletMain->GetWalletTx(txAccount.GetHash());		
			if (IsDarkSilkTxMine(txAccount, "account") && wtxAccountIn != NULL) 
			{
				foundAccount = entry;
	...
	CPubKey ArbiterPubKey(arbiteraccount.vchPubKey);
	CDarkSilkAddress arbiteraddy(ArbiterPubKey.GetID());
	arbiteraddy = CDarkSilkAddress(arbiteraddy.ToString());
	if(!arbiteraddy.IsValid() || !arbiteraddy.isAccount)
		throw runtime_error("Invalid arbiter account or address");
	...
	if(!selleraddy.IsValid() || !selleraddy.isAccount)
		throw runtime_error("Invalid seller account or address");

	CPubKey BuyerPubKey(buyeraccount.vchPubKey);
	...
	arrayOfKeys.push_back(HexStr(arbiteraccount.vchPubKey));
	arrayOfKeys.push_back(HexStr(vchSellerPubKey));
	arrayOfKeys.push_back(HexStr(buyeraccount.vchPubKey));
	...
	int precision = 2;
	CAmount nPricePerUnit = convertCurrencyCodeToDarkSilk(theOffer.vchAccountPeg, theOffer.sCurrencyCode, theOffer.GetPrice(foundAccount), chainActive.Tip()->nHeight, precision;
	...
	SendMoneyForServices(vecSendEscrow, recipientEscrow.nAmount, false, escrowWtx, NULL, NULL, NULL, NULL, false);
	
	// send to seller/arbiter so they can track the escrow through GUI
    // build escrow
    CEscrow newEscrow;
	newEscrow.vchBuyerKey = buyeraccount.vchPubKey;
	newEscrow.vchArbiterKey = arbiteraccount.vchPubKey;
	newEscrow.vchWhitelistAccount = vchWhitelistAccount;
	...
	// if we use an account as input to this escrow tx, we need another utxo for further account transactions on this account, so we create one here
	...
	SendMoneyForServices(vecSend,recipientBuyer.nAmount+ recipientArbiter.nAmount+recipientSeller.nAmount+fee.nAmount, false, wtx, wtxInOffer, wtxInCert, wtxAccountIn, wtxInEscrow);
...
UniValue escrowrelease(const UniValue& params, bool fHelp) {
	...
	if (!GetDarkSilkTransaction(vtxPos.front().nHeight, escrow.escrowInputTxHash, fundingTx, Params().GetConsensus()))
		throw runtime_error("failed to find escrow transaction");

	CPubKey arbiterKey(escrow.vchArbiterKey);
	CDarkSilkAddress arbiterAddress(arbiterKey.GetID());
	if(!arbiterAddress.IsValid())
	...
	try
	{
		...
		strPrivateKey = CDarkSilkSecret(vchSecret).ToString();
	}
	catch(...)
	{
	...
	SendMoneyForServices(vecSend, recipient.nAmount+fee.nAmount, false, wtx, wtxInOffer, wtxInCert, wtxInAccount, wtxIn);
...
UniValue escrowclaimrelease(const UniValue& params, bool fHelp) {
    ...
    for (unsigned int idx = 0; idx < vouts.size(); idx++) {
		...
		for (unsigned int idx = 0; idx < addresses.size(); idx++) {
			...
			CDarkSilkAddress payoutAddress(strAddress);
	...
	if (!pwalletMain->GetKey(keyID, vchSecret))
		throw runtime_error("Private key for seller address " + sellerAddress.ToString() + " is not known");
	const string &strPrivateKey = CDarkSilkSecret(vchSecret).ToString();
	if(!foundSellerPayment)
...
UniValue escrowcomplete(const UniValue& params, bool fHelp) {
	...
	CPubKey buyerKey(escrow.vchBuyerKey);
	CDarkSilkAddress buyerAddress(buyerKey.GetID());
	buyerAddress = CDarkSilkAddress(buyerAddress.ToString());
	if(!buyerAddress.IsValid() || !buyerAddress.isAccount )
		throw runtime_error("Buyer address is invalid!");

	UniValue acceptParams(UniValue::VARR);
	acceptParams.push_back(buyerAddress.accountName);
...
UniValue escrowclaimrefund(const UniValue& params, bool fHelp) {
	...
	if (!pwalletMain->GetKey(keyID, vchSecret))
		throw runtime_error("Private key for buyer address " + buyerAddress.ToString() + " is not known");
	string strPrivateKey = CDarkSilkSecret(vchSecret).ToString();
	if(!foundBuyerPayment)
...
UniValue escrowinfo(const UniValue& params, bool fHelp) {
	...
	oEscrow.push_back(Pair("seller", selleraddy.accountName));
	oEscrow.push_back(Pair("arbiter", arbiteraddy.accountName));
	oEscrow.push_back(Pair("buyer", buyeraddy.accountName));
	...
	string sTotal = strprintf("%llu DRKSLK", (ca.nPricePerUnit/COIN)*ca.nQty);
...
UniValue escrowlist(const UniValue& params, bool fHelp) {
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet)
    {
		...
		else
		{
			escrow = vtxPos.back();
			CTransaction tx;
			if (!GetDarkSilkTransaction(escrow.nHeight, escrow.txHash, tx, Params().GetConsensus()))
				continue;
		...
		oName.push_back(Pair("seller", selleraddy.accountName));
		oName.push_back(Pair("arbiter", arbiteraddy.accountName));
		oName.push_back(Pair("buyer", buyeraddy.accountName));
		...
		string sTotal = strprintf("%llu DRKSLK", (escrow.nPricePerUnit/COIN)*escrow.nQty);
...
UniValue escrowhistory(const UniValue& params, bool fHelp) {
...

    {
        ...
        BOOST_FOREACH(txPos2, vtxPos) {
			...
			string sTotal = strprintf("%llu DRKSLK", (txPos2.nPricePerUnit/COIN)*txPos2.nQty);
			oEscrow.push_back(Pair("total", sTotal));
...
UniValue escrowfilter(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() > 5)
        throw runtime_error(
                "escrowfilter [[[[[search string] maxage=36000] from=0] nb=0] stat]\n"
                        ...
                        "[search string] : Find arbiter or seller via account name or an escrow GUID, empty means all escrows\n"
    ...
    BOOST_FOREACH(pairScan, escrowScan) {
		...
		CDarkSilkAddress buyeraddy(BuyerPubKey.GetID());
		buyeraddy = CDarkSilkAddress(buyeraddy.ToString());
		string buyerAccountLower = buyeraddy.accountName;
		string sellerAccountLower = selleraddy.accountName;
		string arbiterAccountLower = arbiteraddy.accountName;
		boost::algorithm::to_lower(buyerAccountLower);
		boost::algorithm::to_lower(sellerAccountLower);
		boost::algorithm::to_lower(arbiterAccountLower);

        if (strSearch != "" && strSearch != escrow && strSearchLower != buyerAccountLower && strSearchLower != sellerAccountLower && strSearchLower != arbiterAccountLower)
            continue;
        ...
        if (nCountFrom < nFrom + 1)
            continue;
        CTransaction tx;
		if (!GetDarkSilkTransaction(txEscrow.nHeight, txEscrow.txHash, tx, Params().GetConsensus()))
			continue;
		...
		string sTotal = strprintf("%llu DRKSLK", (txEscrow.nPricePerUnit/COIN)*txEscrow.nQty);
		oEscrow.push_back(Pair("total", sTotal));
        oRes.push_back(oEscrow);
...
UniValue escrowscan(const UniValue& params, bool fHelp) {
    ...
    BOOST_FOREACH(pairScan, escrowScan) {
		...
		if (!GetDarkSilkTransaction(nHeight, txEscrow.txHash, tx, Params().GetConsensus()))
			continue;
		...
		oEscrow.push_back(Pair("offer", stringFromVch(txEscrow.vchOffer)));
		oEscrow.push_back(Pair("offeracceptlink", stringFromVch(txEscrow.vchOfferAcceptLink)));
		string sTotal = strprintf("%ll DRKSLK", (txEscrow.nPricePerUnit/COIN)*txEscrow.nQty);
		oEscrow.push_back(Pair("total", sTotal));
		oEscrow.push_back(Pair("expired", expired));
