...
bool AddDarkSilkServicesToDB(const CBlock& block, const CCoinsViewCache& inputs, int nHeight)
{
    vector<vector<unsigned char> > vvchArgs;
    int offerOp;
    ...
    // first pass check cert/escrow/account inputs which are dependent to other services, second pass do the rest
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        {   
            ...
            {
                good = CheckAccountInputs(tx, op, nOut, vvchArgs, inputs, fJustCheck, nHeight, &block);
            }
            if(DecodeCertTx(tx, op, nOut, vvchArgs))
            {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if(tx.nVersion == GetDarkSilkTxVersion())
        {
            ...
            {   
                ...
                // so we can use one account input attached which proves to the network that you are on the whitelist of the root merchant offer owner, we can only use 1 input per block, so we need to group all of the accepts in a block into one tx
                for (unsigned int j = 0; j < tx.vout.size(); j++)
                {
                    if (!IsDarkSilkScript(tx.vout[j].scriptPubKey, offerOp, offerVvch))
                        continue;
...
// Disconnect service related blocks
bool DisconnectAccount(const CBlockIndex *pindex, const CTransaction &tx, int op, vector<vector<unsigned char> > &vvchArgs) 
{
    ...
    paccountdb->ReadAccount(vvchArgs[0], vtxPos);
    CAccountIndex foundAccount = vtxPos.back();
    while (vtxPos.back().txHash == tx.GetHash())    
        vtxPos.pop_back();
    
    CPubKey PubKey(foundAccount.vchPubKey);
    CDarkSilkAddress address(PubKey.GetID());

    if(!paccountdb->WriteAccount(vvchArgs[0], vchFromString(address.ToString()), vtxPos))
        return error("DisconnectBlock() : failed to write to account DB");
    if(fDebug)
        LogPrintf("DISCONNECTED ACCOUNT TXN: account=%s op=%s hash=%s  height=%d\n",
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Block sizes vary wioth the demand for darksilk transaction services
    uint64_t nDRKSLKBlockTx = 0;
    CAmount nDRKSLKRegenFees = 0;
    CAmount nFees = 0;
    int nInputs = 0;
    unsigned int nSigOps = 0;
    ...
    {
        const CTransaction &tx = block.vtx[i];
        // Block sizes vary based on the opreturn value set when creating the service(user)
        if(tx.nVersion == GetDarkSilkTxVersion())
        {
            nDRKSLKBlockTx++;
            if(nDRKSLKBlockTx >= 5)
            {
                ...
                if (nOut != -1)
                    nDRKSLKRegenFees += tx.vout[nOut].nValue*2;
            }
        }
        nInputs += tx.vin.size();
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // DARKSILK
            if (!CheckDarkSilkInputs(tx, view, pindex->nHeight))
                return error("ConnectBlock(): CheckDarkSilkInputs on %s failed",tx.GetHash().ToString());            
            control.Add(vChecks);
        }
    ...
    // DARKSILK Add dynamic block variability baseed on service demand
    CAmount blockReward = nDRKSLKRegenFees + nFees + GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus());
